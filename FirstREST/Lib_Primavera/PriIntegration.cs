...
namespace FirstREST.Lib_Primavera
{
    public class PriIntegration
    {
        ...
        public static List<Model.Lead> LeadsVendedor(string id)
        {
            ...
            if (PriEngine.InitializeCompany(FirstREST.Properties.Settings.Default.Company.Trim(), FirstREST.Properties.Settings.Default.User.Trim(), FirstREST.Properties.Settings.Default.Password.Trim()) == true)
            {
                objList = PriEngine.Engine.Consulta("SELECT ID, Descricao, Entidade, TipoEntidade, EstadoVenda, ValorTotalOV, DataFecho FROM CabecOportunidadesVenda WHERE Vendedor = " + "\'" + id + "\'");
                ...
                while (!objList.NoFim())
                {
                    listLeads.Add(new Model.Lead
                    {
                        ...
                        DataFecho = objList.Valor("DataFecho")
                    });
                    objList.Seguinte();
